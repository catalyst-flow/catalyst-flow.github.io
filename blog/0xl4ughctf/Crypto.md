...
首先RSA的问题一般为质因数分解模数n，这里的n为三个质数相乘$pq(2wp-1)$，其中`w=getPrime(20)`可以尝试求解时爆破。

这一篇[blog](https://blog.csdn.net/wuyunfeng233/article/details/142326133)给出了RSA各种解题思路很好的参考。然后已知$2wp$一个因数为2，$w$可以爆破，可以选取适当的v作为初始值。脚本如下：
```python
from Crypto.Util.number import *
from gmpy2 import *
...
while p<2**20:
    prime.append(p)
    p=next_prime(p) # 确定所有可能的w
 
def attack(n):
    for v in range(3,100):
        v=mlucas(v,2,n)              # 已知2是一个因数，这两步本质上是找到一个合适的初始序列
        v=mlucas(v,n,n)
        for p in prime:            
            v = mlucas(v, p, n)
            g = GCD(v - 2, n)
...
print(flag)
```
总而言之又是一种新的方法。但其实怎么选取初始的`v`这个问题我还搞得不是很明白。